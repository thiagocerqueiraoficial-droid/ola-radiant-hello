    ...
    <div style={{ borderBottom: `1px solid ${RULE}`, background: PAPER }}>
      <div className="mx-auto max-w-[1100px] px-5 py-3 grid grid-cols-3 items-center">
        ...
        <p
          ...
          style={{ ...display, fontWeight: 800, color: INK, fontSize: 22, letterSpacing: "-0.01em" }}
        >
          Mercado em Foco
        </p>
        <div
          className="text-right flex items-center justify-end gap-4"
          style={{ ...meta, color: INK_SOFT, fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", fontWeight: 600 }}
        >
          <span>Assine</span>
...
function SectionNav() {
  return (
    <div style={{ borderBottom: `1px solid ${RULE}`, background: PAPER }}>
      <div
        className="mx-auto max-w-[1100px] px-5 py-2.5"
        style={{ ...meta, color: INK_SOFT, fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 600 }}
      >
...
function Headline() {
  return (
    <h1
      className="mx-auto max-w-[820px] mt-6 text-[34px] sm:text-[44px] md:text-[56px]"
      ...
      <div className="flex items-center gap-2" aria-hidden>
        {["f", "X", "in", "✉"].map((s, i) => (
          <span
            ...
            style={{
              width: 30, height: 30, borderRadius: 999, border: `1px solid ${RULE}`,
              ...meta, fontSize: 11, color: INK_SOFT, fontWeight: 600,
            }}
          >
            {s}
          </span>
...
function H2({ children }: { children: React.ReactNode }) {
  return (
    <h2
      ...
      style={{ ...display, color: INK, fontSize: 30, fontWeight: 700, lineHeight: 1.15 }}
    >
      {children}
    </h2>
  );
...
function PullQuote() {
  return (
    <blockquote
      ...
      style={{ borderLeft: `4px solid ${ALERT}` }}
    >
      <p
        style={{ ...display, color: INK, fontSize: 26, fontStyle: "italic", lineHeight: 1.35, fontWeight: 700 }}
      >
      ...
      <cite
        ...
        style={{ ...meta, color: INK_SOFT, fontSize: 12, fontStyle: "normal", letterSpacing: "0.06em" }}
      >
...
function HowItWorksBox() {
  return (
    <aside
      className="my-10 p-7"
      style={{ background: PAPER2, borderLeft: `4px solid ${ACCENT}` }}
    >
      <p
        style={{ ...meta, color: ACCENT, fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 700 }}
      >
        Como funciona o acesso
      </p>
      <p className="mt-3" style={{ ...serif, color: INK, fontSize: 17, lineHeight: 1.6 }}>
        A entrada na sala ao vivo de {NOME_TRADER} é feita por um grupo gratuito no Telegram. Após ingressar, o operador recebe avisos antes das lives, tem acesso ao curso completo e pode acompanhar operações diárias. Não há cobrança para entrar, e não há upsell imediato.
      </p>
      <a
        href={LINK_TELEGRAM}
        className="inline-block mt-5"
        style={{
          ...meta, background: ACCENT, color: "#fff", fontWeight: 600, fontSize: 14,
          padding: "12px 22px", letterSpacing: "0.04em",
        }}
      >
...
function InlineCta() {
  return (
    <aside
      className="mx-auto max-w-[760px] mt-14 p-8"
      style={{ background: PAPER2, border: `1px solid ${RULE}` }}
    >
      <p
        style={{ ...meta, color: ACCENT, fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", fontWeight: 700 }}
      >
        Conteúdo relacionado
      </p>
      <h3
        className="mt-3"
        style={{ ...display, color: INK, fontSize: 28, fontWeight: 700, lineHeight: 1.2 }}
      >
        Acompanhe as próximas lives diárias gratuitamente
      </h3>
      <p className="mt-3" style={{ ...serif, color: INK_SOFT, fontSize: 17, lineHeight: 1.55 }}>
        O grupo no Telegram é aberto e a próxima sessão começa em algumas horas.
      </p>
      <a
        href={LINK_TELEGRAM}
        className="inline-block mt-6"
        style={{
          ...meta, background: ACCENT, color: "#fff", fontWeight: 600, fontSize: 15,
          padding: "16px 28px", letterSpacing: "0.04em",
        }}
      >
...
function ReadAlso() {
  ...
  return (
    <section
      className="mx-auto max-w-[1100px] mt-20 px-5 pb-12"
      style={{ borderTop: `1px solid ${RULE}`, paddingTop: 32 }}
    >
      <p
        style={{ ...meta, color: INK_SOFT, fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", fontWeight: 700 }}
      >
        Leia também
      </p>
      <div className="mt-5 grid md:grid-cols-3 gap-6">
        {items.map((t, i) => (
          <article key={i} className="pt-5" style={{ borderTop: `1px solid ${RULE}` }}>
            <p
              style={{ ...meta, color: ACCENT, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 700 }}
            >
              Economia
            </p>
            <h4
              className="mt-2"
              style={{ ...display, color: INK, fontSize: 20, fontWeight: 700, lineHeight: 1.2 }}
            >
              {t}
            </h4>
...
function Body() {
  return (
    <article className="mx-auto max-w-[760px] px-5 mt-10">
      ...
      <P>
        A taxa de acerto reportada nas lives é de 99%, sem uso de estratégias comuns entre operadores frequentes de opções binárias como gale ou proteção dobrada. O modelo tem chamado atenção por um motivo simples: é verificável em tempo real. Qualquer pessoa com acesso ao grupo pode assistir, acompanhar entrada, saída e resultado de cada operação.
      </P>
      ...
      <P>
        O modelo de sala aberta entrou em conflito com boa parte dos players tradicionais do setor. Vendedores de sinais pagos, por exemplo, dependem da opacidade do resultado para sustentar o valor cobrado. Quando um operador demonstra que é possível acompanhar operações reais gratuitamente, a assimetria do mercado diminui.
      </P>
      <P>
      ...
      <P>
        {NOME_TRADER} evita comentar diretamente sobre concorrentes. Em entrevista, ele se limita a dizer que o modelo “se vende sozinho quando a pessoa assiste uma live”. A Alpha Academy não divulga faturamento, mas afirma que cerca de 7% dos membros do grupo gratuito migram para produtos pagos ao longo do tempo.
      </P>

      <H2>O que esperar daqui pra frente</H2>
      <P>
        Para 2026, a academia projeta expansão da base de operadores e lançamento de um programa de formação avançada voltado para quem já opera com consistência. Por enquanto, a porta de entrada continua sendo o grupo gratuito, que mantém limite técnico de membros por sala e abre vagas em lotes conforme demanda.
      </P>
      <P>
        Para quem tem curiosidade sobre o modelo, o acesso à sala ao vivo permanece disponível sem custo. A recomendação do próprio trader é simples: entrar, assistir uma ou duas lives, e julgar pelo que for visto em tempo real.
      </P>
    </article>
  );
}
